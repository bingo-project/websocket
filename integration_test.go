...
func TestFullMiddlewareChain(t *testing.T) {
	...
	var order []string
	router.Use(
		func(next websocket.Handler) websocket.Handler {
	...
	// Public handler
	router.Handle("public.test", func(c *websocket.Context) *jsonrpc.Response {
	...
	private.Handle("private.test", func(c *websocket.Context) *jsonrpc.Response {
...
func TestMiddlewareChain_ExecutionOrder(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		n := i // capture
		router.Use(func(next websocket.Handler) websocket.Handler {
			return func(c *websocket.Context) *jsonrpc.Response {
	...
	router.Handle("test", func(c *websocket.Context) *jsonrpc.Response {
		order = append(order, "handler")
...
func TestGroupMiddlewareIsolation(t *testing.T) {
	...
	public := router.Group()
	public.Handle("public.test", func(c *websocket.Context) *jsonrpc.Response {
		return jsonrpc.NewResponse(c.Request.ID, "public")
	})

	// Private group with additional middleware
	private := router.Group(func(next websocket.Handler) websocket.Handler {
		return func(c *websocket.Context) *jsonrpc.Response {
