...
func (c *Client) handleMessage(data []byte) {
	...
	if err := json.Unmarshal(data, &req); err != nil {
		resp := jsonrpc.NewErrorResponse(nil,
			NewError(400, "ParseError", "Invalid JSON: %s", err.Error()))
		c.sendJSON(resp)
	...
	if c.router == nil {
		resp := jsonrpc.NewErrorResponse(req.ID,
			NewError(500, "InternalError", "Router not configured"))
		c.sendJSON(resp)
...
func (c *Client) ParseToken(token string) (*TokenInfo, error) {
	if c.tokenParser == nil {
		return nil, NewError(500, "InternalError", "Token parser not configured")
	}
