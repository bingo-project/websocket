...
type RateLimiterStore struct {
	mu       sync.RWMutex
	limiters map[*websocket.Client]map[string]*rate.Limiter
}

// NewRateLimiterStore creates a new rate limiter store.
func NewRateLimiterStore() *RateLimiterStore {
	return &RateLimiterStore{
		limiters: make(map[*websocket.Client]map[string]*rate.Limiter),
	}
}

func (s *RateLimiterStore) get(client *websocket.Client, method string, limit float64) *rate.Limiter {
	s.mu.RLock()
		...
		return func(c *websocket.Context) *jsonrpc.Response {
			...
			if !limiter.Allow() {
				return jsonrpc.NewErrorResponse(c.Request.ID,
					websocket.NewError(429, "TooManyRequests", "Rate limit exceeded"))
			}
